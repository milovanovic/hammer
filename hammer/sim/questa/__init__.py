#  hammer-vlsi plugin for Questa Sim
...
class questa(HammerSimTool, DummyHammerTool):
    ...
    def qhsim(self) -> bool:
        ...
        level = self.get_setting("sim.inputs.level")  # undefined, syn, or par
        ...
        if level in ["syn", "par"]:  # add Verilog models of standard cells
        ...
        if timing_annotated:
            ...
            if not self.sdf_file:  # if SDF file is not specified in input .yml
                if level == "par":
